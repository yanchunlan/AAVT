...
import com.example.mp4processor.utils.MatrixUtils;
...
public abstract class BaseFilter implements Renderer {
    ...
    private final LinkedList<Runnable> mTasks = new LinkedList<>();         // 添加进去task 的都会在draw的时候绘制出来，并保证执行一次
    ...
    public void draw(int texture, long time) {
        draw(texture);
        onDrawChild(time);
    }
     ...
     * @param texture
     */
    private void onDrawDestroy(int texture) {
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, 0);
//        int []ids = new int[]{texture};
